...
public class JpaManager {
    ...
    public synchronized <T> void persist(T entity) {
        this.transaction((em) -> em.persist(entity));
    }

    public synchronized <T> void persist(List<T> entities) {
        this.transaction((em) -> entities.forEach(em::persist));
    }

    public synchronized <T> void update(T entity) {
        this.transaction((em) -> em.merge(entity));
    }
    ...
    public synchronized <T> void remove(T entity) {
        this.transaction((em) -> em.remove(entity));
    }
}
